...
for (i in colnames(pred_puf17)){
  temp <- table(pred_puf17[[i]])
  if (nrow(temp) < 15) {
    ...
    write(temp_table, file =  "postPredictors17.tex", append = TRUE)
  }
}

# Recoded outcomes
# Appends each variable to file as an additional table
for (i in colnames(outcomes_puf17)){
  temp <- table(outcomes_puf17[[i]])
  if (nrow(temp) < 15) {
    # Get latex code of table without printing to console
    temp_table <- capture.output(print(xtable(temp, caption = i, type = "latex")))
    ...
    # Append latex code to file
    write(temp_table, file =  "postOutcomes17.tex", append = TRUE)
  }
}

# Predictors and outcomes before recoding
# Appends each variable to file as an additional table
for (i in colnames(pre17)){
  temp <- table(pre17[[i]])
  if (nrow(temp) < 15) {
    # Get latex code of table without printing to console
    temp_table <- capture.output(print(xtable(temp, caption = i, type = "latex")))
    ...
    # Append latex code to file
    write(temp_table, file = "pre17.tex", append = TRUE)
  }
}
...
pred_sum_testing_puf17 <- transmute(pred_puf17,
...
outcome_sum_testing_puf17 <- transmute(outcomes_puf17,

                      # Discharge Destination
                      discharge = discharge_unknown + discharge_skilled + discharge_unskilled + discharge_facility + discharge_home + discharge_acute + discharge_rehab + discharge_expired + discharge_hospice + discharge_multi,
...
#### Export to CSV ####
save(pred_sum_testing_puf17, file = "pred_sum_testing_puf17.Rda")
save(outcome_sum_testing_puf17, file =  "outcome_sum_testing_puf17.Rda")
